...
def index():
    if request.method == 'GET':
        ...
        # Validate all input
        ticker, values = None, None
        if 'ticker' in request.form and request.form['ticker'] in codes:
            ticker = request.form['ticker'].upper()
        if 'values' in request.form:
            values = request.form.getlist('values')
            for value in values:
                if value not in ['open', 'close', 'high', 'low']:
                    values = None
                    break
            if values is not None and 'adjusted' in request.form:
                for i, value in enumerate(values):
                    values[i] = 'adj_' + value
        if ticker is None or values is None:
        ...
        return render_template('index.html', plot_div=div.strip(), plot_script=script.strip())
    else:
